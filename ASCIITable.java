...
public class ASCIITable{
	...
	private void drawRow(StringBuilder output, int index){
		...
		for(int i = 0; i < this.cols; i++){
			this.drawCell(rowString, row[i], i, height);
	...
	private void drawCell(StringBuilder row, String value, int col, int rowHeight){
		int colWidth = this.colWidth[col] == 0 ? value.length() : this.colWidth[col];
		String[] lines = getLines(value, colWidth);
	}
	 ...
	 */
	private static int getLinesCount(String value, double width){
	...
	private static String[] alignCellText(String[] lines, int width, int height, int alignment){
		String[] rows = new String[height];
		int verticalPadding = height - lines.length;
	}
}